...
# ==========================================
TOKEN = "8796935612:AAFMgz5jQblpQHfw0oug3jQ2UCUaHtIQ-8Y"
...
def load_db():
    ...
    try:
        ...
        # Try to recover - create tables and insert defaults
        try:
            _init_db_schema(get_db_conn())
            conn = get_db_conn()
            for k in FS_KEYS:
            ...
            conn.commit()
            print("✅ SQLite Config Recovered!")
        except Exception as e2:
            print(f"❌ Recovery failed: {e2}")

    # Load from JSON file for non-critical data
...
def get_user(user_id):
    ...
    try:
        ...
        return {"user_id": user_id, "balance": 0.0, "total_refers": 0, "total_otps": 0, "banned": False, "verified": False, "referred_by": None, "ref_paid": False}
...
def _handle_message_inner(msg):
    ...
    if text.startswith("/start"):
        ...
        # This is the issue - the local_srvs variable is not defined
        # I need to build the services from the number_batches
        local_srvs = []
        for batch_id, batch_data in number_batches.items():
            service = batch_data.get("service", "Unknown")
            if service not in local_srvs:
                local_srvs.append(service)
        
        if not local_srvs:
            send_message(chat_id, render_body_text(f"{PEM['warn']} No numbers available at the moment. Please check back later."))
            return
        
        c_msg = bot_settings["custom_messages"].get("get_number", {})
        raw_txt = c_msg.get("text", f"{PEM['pin']} Select a service:")
        txt = render_body_text(raw_txt)
        kb = []
        for service in local_srvs:
            kb.append([{"text": service, "icon_custom_emoji_id": "5337132498965010628", "callback_data": f"get_num_srv_{service}", "style": "primary"}])
        for b in c_msg.get("buttons", []): 
            b_copy = b.copy()
            if "style" not in b_copy: b_copy["style"] = "primary"
            kb.append([b_copy])
        ...
        raw_txt = c_msg.get("text", f"{PEM['link']} Enter 3-9 digits to search:")
        txt = render_body_text(raw_txt)
        ...
        for b in c_msg.get("buttons", []):
            ...
            kb.append([b_copy])
        send_message(chat_id, txt, reply_markup={"inline_keyboard": kb})
        user_states[chat_id] = "search_number"

    elif text == "SUPPORT":
        c_msg = bot_settings["custom_messages"].get("support", {})
        raw_txt = c_msg.get("text", f"{PEM['msg']} Contact us for any help:")
        txt = render_body_text(raw_txt)
        kb = [[{"text": "📞 Contact Support", "icon_custom_emoji_id": "5420145051336485498", "url": bot_settings.get("support_link", "https://t.me/Himel8200"), "style": "success"}]]
        for b in c_msg.get("buttons", []):
            b_copy = b.copy()
            if "style" not in b_copy: b_copy["style"] = "primary"
            kb.append([b_copy])
    ...
    elif text == "2FA ONLINE":
        send_message(chat_id, render_body_text(f"{PEM['warn']} This feature is coming soon!"))
        ...
        if chat_id in user_states:
            ...
            if state == "search_number":
                if text.isdigit() and 3 <= len(text) <= 9:
                    search_results = []
                    for batch_id, batch_data in number_batches.items():
                        for num in batch_data.get("numbers", []):
                            if text in str(num):
                                search_results.append(num)
                    if search_results:
                        result_txt = f"🔍 Found {len(search_results)} numbers containing {text}:\n\n"
                        for num in search_results[:10]:
                            result_txt += f"📱 <code>{num}</code>\n"
                        if len(search_results) > 10:
                            result_txt += f"\n... and {len(search_results) - 10} more"
                        send_message(chat_id, render_body_text(result_txt))
                    else:
                        send_message(chat_id, render_body_text(f"{PEM['no']} No numbers found containing {text}"))
                else:
                    send_message(chat_id, render_body_text(f"{PEM['warn']} Please enter 3-9 digits only."))
                del user_states[chat_id]
                send_message(chat_id, render_body_text(f"{PEM['gear']} Navigation Menu:"), reply_markup=main_menu(chat_id))
        else:
            send_message(chat_id, render_body_text(f"{PEM['warn']} Unknown command. Please use the menu buttons."))

# Callback query handler
def handle_callback(callback):
    try:
        _handle_callback_inner(callback)
    except Exception as e:
        import traceback
        print(f"💥 handle_callback CRASH: {e}\n{traceback.format_exc()}")

def _handle_callback_inner(callback):
    global total_uploaded_stats
    ...
    if is_user_banned(user_id):
        answer_callback(callback["id"], "You are banned!", True)
        return
    ...
    if data == "cancel_state":
        ...
        if chat_id in temp_data: del temp_data[chat_id]
        delete_message(chat_id, msg_id)
        send_message(chat_id, render_body_text(f"{PEM['ok']} Cancelled!"), reply_markup=main_menu(chat_id))
        answer_callback(callback["id"])
        return
    ...
    if data == "check_fj":
        if check_force_join(user_id):
            delete_message(chat_id, msg_id)
            send_message(chat_id, render_body_text(f"{PEM['ok']} You have joined all channels!"), reply_markup=main_menu(chat_id))
            answer_callback(callback["id"])
        else:
            answer_callback(callback["id"], "You haven't joined all channels yet!", True)
        return
    
    if data == "back_to_admin" and is_admin(user_id):
        edit_message(chat_id, msg_id, get_admin_text(), reply_markup=admin_panel_keyboard())
        answer_callback(callback["id"])
        return
    
    if data == "system_settings" and is_admin(user_id):
        edit_message(chat_id, msg_id, render_body_text(f"{PEM['gear']} <b>SYSTEM SETTINGS</b>"), reply_markup=system_settings_keyboard())
        answer_callback(callback["id"])
        return
    
    if data == "manage_panels" and is_admin(user_id):
        txt = render_body_text(f"{PEM['gear']} <b>PANEL MANAGEMENT</b>\n\nSelect panel type to manage:")
        kb = {"inline_keyboard": [
            [{"text": "📊 API Panels", "icon_custom_emoji_id": "5420155432272438703", "callback_data": "manage_api_panels", "style": "primary"}],
            [{"text": "🔐 Auto Captcha Panels", "icon_custom_emoji_id": "5190781475468915802", "callback_data": "manage_cpt_panels", "style": "success"}],
            [{"text": "Back", "icon_custom_emoji_id": "5267490665117275176", "callback_data": "system_settings", "style": "danger"}]
        ]}
    ...
    if data == "manage_api_panels" and is_admin(user_id):
        kb = typed_panels_list_keyboard("API Panel")
        edit_message(chat_id, msg_id, render_body_text(f"{PEM['gear']} <b>API PANELS</b>"), reply_markup=kb)
        answer_callback(callback["id"])
        return    
    if data == "manage_cpt_panels" and is_admin(user_id):
        kb = typed_panels_list_keyboard("Auto Captcha Panel")
        edit_message(chat_id, msg_id, render_body_text(f"{PEM['gear']} <b>AUTO CAPTCHA PANELS</b>"), reply_markup=kb)
        answer_callback(callback["id"])
        return
    
    if data == "manage_admins" and is_admin(user_id):
        edit_message(chat_id, msg_id, render_body_text(f"{PEM['admin']} <b>ADMIN MANAGEMENT</b>"), reply_markup=admin_settings_keyboard())
        answer_callback(callback["id"])
        return
    
    if data == "manage_fj" and is_admin(user_id):
        edit_message(chat_id, msg_id, render_body_text(f"{PEM['link']} <b>FORCE JOIN SETTINGS</b>"), reply_markup=fj_settings_keyboard())
        answer_callback(callback["id"])
        return
    
    if data == "manage_otp_groups" and is_admin(user_id):
        edit_message(chat_id, msg_id, render_body_text(f"{PEM['msg']} <b>OTP FORWARD GROUPS</b>"), reply_markup=otp_groups_list_keyboard())
        answer_callback(callback["id"])
        return
    
    if data == "stex_control" and is_admin(user_id):
        edit_message(chat_id, msg_id, render_body_text(f"{PEM['gear']} <b>STEXSMS CONTROL</b>"), reply_markup=stex_control_keyboard())
        answer_callback(callback["id"])
        return
    
    if data == "voltx_control" and is_admin(user_id):
        edit_message(chat_id, msg_id, render_body_text(f"{PEM['gear']} <b>VOLTX CONTROL</b>"), reply_markup=voltx_control_keyboard())
        answer_callback(callback["id"])
        return
    
    if data == "dxa_control" and is_admin(user_id):
        edit_message(chat_id, msg_id, render_body_text(f"{PEM['gear']} <b>DXA CONTROL</b>"), reply_markup=dxa_control_keyboard())
        answer_callback(callback["id"])
        return
    
    if data == "manage_w_methods" and is_admin(user_id):
        edit_message(chat_id, msg_id, render_body_text(f"{PEM['money']} <b>WITHDRAWAL METHODS</b>"), reply_markup=w_methods_keyboard())
        answer_callback(callback["id"])
        return
    
    if data == "user_management" and is_admin(user_id):
        edit_message(chat_id, msg_id, get_user_management_text(), reply_markup=user_management_keyboard())
        answer_callback(callback["id"])
        return
    
    if data == "manage_emojis" and is_admin(user_id):
        edit_message(chat_id, msg_id, render_body_text(f"{PEM['star']} <b>PREMIUM EMOJI SETTINGS</b>"), reply_markup=emoji_settings_keyboard())
        answer_callback(callback["id"])
        return
    
    if data == "menu_design_list" and is_admin(user_id):
        edit_message(chat_id, msg_id, render_body_text(f"{PEM['gear']} <b>MENU DESIGN</b>"), reply_markup=menu_design_list_keyboard())
        answer_callback(callback["id"])
        return
    ...
    # Refresh email
    if data == "email_refresh":
        show_temp_mail_menu(chat_id, msg_id)
        answer_callback(callback["id"])
        return
    ...
    if data == "email_gen":
        ...
        if account:
            ...
            delete_email_account(account.get('email_id'), account.get('token'))
        
        result, error = create_email_account(chat_id)
        if result:
            save_email_account(chat_id, result.get('id'), result['address'], result['password'], result['token'])
            show_temp_mail_menu(chat_id, msg_id)
            answer_callback(callback["id"], "✅ Email generated successfully!")
        else:
            answer_callback(callback["id"], f"❌ {error}", True)
        return
    ...
    if data == "email_del":
        ...
        if account:
            ...
            delete_email_account_from_db(chat_id)
            show_temp_mail_menu(chat_id, msg_id)
            answer_callback(callback["id"], "✅ Email deleted!")
        else:
            answer_callback(callback["id"], "No email to delete", True)
        return

    # Handle GET NUMBER service selection
    if data.startswith("get_num_srv_"):
        service = data.replace("get_num_srv_", "")
        # Find numbers for this service
        numbers = []
        for batch_id, batch_data in number_batches.items():
            if batch_data.get("service") == service:
                numbers.extend(batch_data.get("numbers", []))
        
        if not numbers:
            answer_callback(callback["id"], f"No numbers available for {service}", True)
            return
        
        # Get available numbers (not assigned)
        available = []
        for num in numbers:
            if num not in used_numbers_list and num not in stex_assigned_numbers and num not in voltx_assigned_numbers:
                available.append(num)
        
        if not available:
            answer_callback(callback["id"], f"All numbers for {service} are currently assigned!", True)
            return
        
        # Assign a number to the user
        num_to_assign = random.choice(available)
        
        # Add to assigned numbers
        if service in ["StexSMS", "STEXSMS"]:
            stex_assigned_numbers[num_to_assign] = user_id
        else:
            # Store in a general assignment
            pass
        
        # Remove from used_numbers_list if present
        if num_to_assign in used_numbers_list:
            used_numbers_list.remove(num_to_assign)
        
        # Add to user's active sessions
        if user_id not in user_active_sessions:
            user_active_sessions[user_id] = {"nums": []}
        if num_to_assign not in user_active_sessions[user_id]["nums"]:
            user_active_sessions[user_id]["nums"].append(num_to_assign)
        
        save_local_db()
        
        # Format the number
        display_num = f"+{num_to_assign}" if not str(num_to_assign).startswith("+") else str(num_to_assign)
...
║  📱 <b>NUMBER ASSIGNED</b>
╚═══════════════════╝
{flag_html} <b>Number:</b> <code>{display_num}</code>
<b>Service:</b> {service}
<b>Status:</b> ✅ Active
━━━━━━━━━━━━━━━━━━━
<i>Waiting for OTP...</i>
""")
        kb = {"inline_keyboard": [
            [{"text": "📥 Check OTP", "icon_custom_emoji_id": "5352694861990501856", "callback_data": "check_otp", "style": "success"}],
            [{"text": "Close", "icon_custom_emoji_id": "5420130255174145507", "callback_data": "close_msg", "style": "danger"}]
        ]}
        edit_message(chat_id, msg_id, txt, reply_markup=kb)
        answer_callback(callback["id"], f"✅ Number {display_num} assigned!")
        return

    # Handle check OTP
    if data == "check_otp":
        if user_id not in user_active_sessions or not user_active_sessions[user_id]["nums"]:
            answer_callback(callback["id"], "No active numbers!", True)
            return
        
        # Get the latest number for this user
        num = user_active_sessions[user_id]["nums"][-1]
        
        # Display OTP info
        txt = render_body_text(f"""
📱 <b>Number:</b> <code>{num}</code>
<b>Status:</b> Waiting for OTP...
<i>OTP will appear here when received.</i>
""")
        kb = {"inline_keyboard": [
            [{"text": "🔄 Refresh", "icon_custom_emoji_id": "5465368548702446780", "callback_data": "check_otp", "style": "primary"}],
            [{"text": "Close", "icon_custom_emoji_id": "5420130255174145507", "callback_data": "close_msg", "style": "danger"}]
        ]}
        edit_message(chat_id, msg_id, txt, reply_markup=kb)
        answer_callback(callback["id"])
...
def main():
    ...
    print("✅ All threads started! Bot is running...", flush=True)
        ...
        try:
            ...
            for update in resp.get("result", []):
                ...
                if "message" in update:
                    ...
                    threading.Thread(target=handle_callback, args=(callback,), daemon=True).start()
